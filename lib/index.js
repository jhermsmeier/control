...
control.runTasks = function( context, tasks, eventName, callback ) {
  ...
  async.forEach( tasks, ( task, next ) => {
    ...
    if( task.skip ) {
      if( control.reporter.skip ) control.reporter.skip( task )
    ...
    try {
      promise = task.fn( done )
      if( promise ) {
        promise.catch( done ).then(() => done())
      }
    } catch( err ) {
      error = err
    }

    if( error || ( !task.fn.length && !promise ) ) {
      done( error )
    }
...
control.runContext = function( context, callback ) {
  ...
  async.series([
    ...
    ( next ) => {
      ...
      // var eventName = context.only.length ? 'only' : 'test'
      var tasks = context.only.length ? context.only : context.tasks
