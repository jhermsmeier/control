...
control.runTask = function( eventName, task, callback ) {
...
control.runUtil = function( eventName, tasks, callback ) {

  if( tasks == null || !tasks.length )
    return void callback()

  async.forEach( tasks, ( task, next ) => {
    control.runTask( eventName, task, next )
  }, callback )
...
control.runTasks = function( context, tasks, eventName, callback ) {
  ...
  async.forEach( tasks, ( task, next ) => {
    ...
    control.runUtil( 'before', task.context.before, ( error ) => {
      if( error ) return void next( error )
      control.runTask( eventName, task, ( error ) => {
        if( error ) return void next( error )
        control.runUtil( 'after', task.context.after, next )
      })
    })
  ...
  async.series([
    ( next ) => {
      context.setup.length ?
        control.runUtil( 'setup', context.setup, next ) :
        next()
    },
    ( next ) => {
    ...
    ( next ) => {
      context.teardown.length ?
        control.runUtil( 'teardown', context.teardown, next ) :
        next()
    }
  ], ( error ) => {
